...
For code that needs to work across multiple exchanges, use `CryptoExchange.Net.SharedApis` interfaces (`IGetTickerRest`, `IPlaceSpotOrderRest`, etc.) accessed via `.SharedApi` properties. Same pattern works for other exchanges in the CryptoExchange.Net family.
