...
namespace HTX.Net.Objects.Sockets.Queries
{
    internal class HTXOpQuery : Query<HTXOpResponse>
    {
        ...
        {
            _client = client;
            MessageRouter = MessageRouter.CreateForQuery<HTXOpResponse>(((HTXOpMessage)Request).RequestId!, HandleMessage);
        }
        ...
        public CallResult<HTXOpResponse> HandleMessage(SocketConnection connection, DateTime receiveTime, string? originalData, HTXOpResponse message)
        {
            if (message.ErrorCode == 0)
                return CallResult.Ok<HTXOpResponse>(message, originalData);

            return CallResult.Fail<HTXOpResponse>(new ServerError(message.ErrorCode!, _client.GetErrorInfo(message.ErrorCode!, message.ErrorMessage)));
        }
    }
}
