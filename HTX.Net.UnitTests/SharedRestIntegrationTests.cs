...
namespace HTX.Net.UnitTests
{
    ...
    internal class SharedRestIntegrationTests
    {
        ...
        private IHTXRestClientSpotApiShared GetSpotRestClient()
        {
            var collection = new ServiceCollection();
            collection.AddHTX(x => x.Rest.OutputOriginalData = true);
            ...
            return sp.GetRequiredService<IHTXRestClient>().SpotApi.SharedClient;
        }

        private IHTXRestClientUsdtFuturesApiShared GetFuturesRestClient()
        {
            var collection = new ServiceCollection();
            collection.AddHTX(x => x.Rest.OutputOriginalData = true);
            collection.AddLogging(x =>
            ...
            var sp = collection.BuildServiceProvider();
            return sp.GetRequiredService<IHTXRestClient>().UsdtFuturesApi.SharedClient;
        }
        ...
        [Test]
        public async Task TestFuturesMarkKlinesRequests()
        {
            if (!ShouldRun())
                return;

            var client = GetFuturesRestClient();
            var result1 = await client.GetMarkPriceKlinesAsync(new GetKlinesRequest(_futuresSymbol, SharedKlineInterval.OneDay));
            var result3 = await client.GetMarkPriceKlinesAsync(new GetKlinesRequest(_futuresSymbol, SharedKlineInterval.OneDay, DateTime.UtcNow.AddDays(-5), DateTime.UtcNow));
            CheckResults([
                ("FuturesMarkKlines", result1),
                ("FuturesMarkKlinesTimed", result3),
                ]);
        }
