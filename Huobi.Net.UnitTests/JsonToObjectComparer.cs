...
namespace Huobi.Net.UnitTests
{
    public class JsonToObjectComparer<T>
    {
        ...
        {
            var resultProperties = obj.GetType()
                .GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.GetProperty | BindingFlags.Instance | BindingFlags.SetProperty).Select(p => (p, (JsonPropertyAttribute)p.GetCustomAttributes(typeof(JsonPropertyAttribute), true).SingleOrDefault()));
